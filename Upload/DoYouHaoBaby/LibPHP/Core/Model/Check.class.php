...
class Check{
	...
	public static function checkBatch($Data,array $arrChecks,$bCheckAll=true,&$arrFailed=null){
		...
		foreach($arrChecks as $arrV){
			...
			if(!$bResult && !$bCheckAll){return false;}
		}
		return(bool)$bResult;
	}

	public static function checkByArgs($Check,array $arrArgs){
		static $arrInternalFuncs;
		...
		elseif(strpos($Check,'::')){// 使用::回调处理
			...
			self::$_sErrorMessage=G::L('不存在的验证规则');
			self::$_bIsError=true;
			return false;
		}
		if($bResult===false){
			self::$_sErrorMessage=G::L('验证数据出错');
			self::$_bIsError=true;
		}
		return $bResult;
	}
	...
	public static public function isError(){
		return self::$_bIsError;
	}

	public static public  function getErrorMessage(){
