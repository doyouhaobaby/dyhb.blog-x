...
class TreeCategory{
	...
	public function __construct($arrNodes=array()){
		if(!empty($arrNodes)){
			if(E::oneImensionArray($arrNodes)){A::ASSERT_(0);}
	...
	public function getDisplayTreeItem($nCategory,$Callback=null){
		if(is_callable($Callback)){return call_user_func($Callback,$nCategory);}
		else{return $nCategory;}
	}

	public function getDisplayTreeItemLiMore($nCategory){
		if(is_callable($this->_treeLiCallback)){return call_user_func($this->_treeLiCallback,$nCategory);}
		else{return '';}
	}
