...
class Coll implements Iterator,ArrayAccess,Countable{
	...
	public function offsetGet($Offset){
		if(isset($this->_arrColl[$Offset])){return $this->_arrColl[$Offset];}
		G::E(G::L('无效的键名 %s。','LibDyhb',null,$Offset));
	}
	...
	public function offsetSet($Offset,$Value){
		...
		while(isset($this->_arrColl[$Offset])){$Offset++;}
	...
	public function first(){
		if(count($this->_arrColl)){return reset($this->_arrColl);}
		G::E(G::L('%s 集合中没有任何对象。','LibDyhb',null,$this->_sType));
	}
	...
	public function append($Data){
		foreach($Data as $oItem){$this->offsetSet(null,$oItem);}
		return $this;
	}
	...
	public function search($sPropName,$Needle,$bStrict=false){
		foreach($this->_arrColl as $oItem){
			if($bStrict){
				if($oItem->{$sPropName}===$Needle){return $oItem;}
			}
			else{
				if($oItem->{$sPropName}==$Needle){return $oItem;}
			}
	...
	protected function checkType_($oObject){
		if(is_object($oObject)){
			if($oObject instanceof $this->_sType){return;}
			$sType=get_class($oObject);
		}
		else{
			$sType=gettype($oObject);
		}
		G::E(G::L('集合只能容纳 %s 类型的对象，而不是 %s 类型的值.','LibDyhb',null ,$this->_sType,$sType));
	}
