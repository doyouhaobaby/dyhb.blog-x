...
abstract class DbConnect{
	...
	protected $_arrComparison=array('eq'=>'=','neq'=>'!=','gt'=>'>','egt'=>'>=','lt'=>'<','elt'=>'<=','notlike'=>'NOT LIKE','like'=>'LIKE');
	...
	public function addConnect($Config,$nLinkNum=null){
		...
		$this->_hCurrentConnect=$this->commonConnect($Config,$nLinkNum);// 创建新的数据库连接
	...
	public function writeConnect(){
		if($this->_hWriteConnect && is_resource($this->_hWriteConnect)){// 判断是否已经连接
			return $this->_hWriteConnect;
		}
		$hDb=$this->commonConnect($this->_arrWriteDbConfig);// 没有连接开始请求连接
		if(!$hDb || !is_resource($hDb)){// 判断是否连接成功
			return false;
		}
	...
	public function readConnect(){
		...
		if(is_array($this->_arrHReadConnect) && !empty($this->_arrHReadConnect)){// 如果有可用的Slave连接，随机挑选一台Slave
		...
		if(!is_array($this->_arrReadDbConfig) || empty($this->_arrReadDbConfig)){// 连接到所有Slave数据库，如果没有可用的Slave机则调用Master
			return $this->writeConnect();
		}
		$this->_arrHReadConnect=array();// 读服务器连接
		...
		if(!is_array($this->_arrHReadConnect) || empty($this->_arrHReadConnect)){// 如果没有一台可用的Slave则调用Master
			$this->errorMessage('Not availability slave db connection,call master db connection');
			return $this->writeConnect();
		}
		$sKey=array_rand($this->_arrHReadConnect);// 随机在已连接的Slave机中选择一台
		if(isset($this->_arrHReadConnect[$sKey]) && is_resource($this->rdbConn[$sKey])){
			return $this->_arrHReadConnect[$sKey];
		}
		if(count($this->_arrHReadConnect)>1){// 如果选择的slave机器是无效的，并且可用的slave机器大于一台则循环遍历所有能用的slave机器
		...
		return $this->writeConnect();// 如果没有可用的Slave连接，则继续使用Master连接
	}
	...
	public function query($Sql,$sDb=''){
		$sOldDb=$this->getCurrentDb();// 切换到指定数据库
		if($sDb and $sDb != $sOldDb){
			$sOldDB=$this->selectDb($sDb);
		}
		$bRes=$this->query_($Sql);// 执行
		if($sOldDb){// 还原到以前的数据库
			$this->selectDb($sOldDb);
		}
		if($bRes===false){// 错误处理
			G::E(G::L('一条 SQL 语句在执行中出错:%s','DbDyhb',null,$Sql));
		}
		return $bRes;
	}

	public function exec($sSql,$arrInput=null){
		if(is_array($arrInput)){// 如果有给定占位符，解析SQL
		...
		elseif($hResult){
			...
			if($this->getErrorCode()==1062){
				$sMoreMessage=G::L('主键重复','DbDyhb').' Error:<br/>'.G::L('你的操作中出现了重复记录，请修正错误！','DbDyhb');
			}
			G::E($sMoreMessage);
		}
	}
	...
	public function setConnectHandle($hConnectHandle){
		A::RESOURCE($hConnectHandle,G::L('参数 $hConnectHandle 必须是有效的数据库连接','DbDyhb'));
			...
			case Db::PARAM_AT_NAMED:
				...
				for($nOffset=1; $nOffset < $nMax; $nOffset +=2){
					...
					if(!isset($arrParams[$sArgName])){
						G::E(sprintf('Invalid parameter "%s" for "%s"',$sArgName,$sSql));
					}
	...
	public function qualifyWhere($Where,$sTableName=null,$arrFieldsMapping=null,$hCallback=null){
		...
		if(is_string($Where)){// 直接使用字符串条件
			...
			if(array_key_exists('logic_',$Where)){
				$sOperate=' '.strtoupper($Where['logic_']).' ';// 定义逻辑运算规则 例如 OR XOR AND NOT
				unset($Where['logic_']);
			}
			else{
					...
					if(is_array($val)){
						if(isset($val[0]) && is_string($val[0])){
							if(preg_match('/^(EQ|NEQ|GT|EGT|LT|ELT|NOTLIKE|LIKE)$/i',$val[0])){ // 比较运算
								...
								$sWhereStr.=$this->qualifyWhereField($sKey,$sTableName,$arrFieldsMapping,$hCallback).' '.$arrComparison[ strtolower($val[0]) ].' '.(isset($val[1])?$this->qualifyStr($val[1]) :'');
							}
							...
							elseif(preg_match('/BETWEEN/i',$val[0])){ // BETWEEN运算
								...
								$sWhereStr.='('.$this->qualifyWhereField($sKey,$sTableName,$arrFieldsMapping,$hCallback).' BETWEEN '.(isset($arrData[0])?$arrData[0]:'').' AND '.(isset($arrData[1])?$arrData[1] :'').')';
							}
							else{
								G::E(G::L('表达式错误%s','DbDyhb',null,(isset($arrData[0])?$arrData[0]:'')));
							}
						}
						else{
							$nCount=count($val);
	...
	public function qualifyWhereField($sField,$sTableName=null,$arrFieldsMapping=null,$hCallback=null){
		...
		$sField='['.trim($sField,'[]').']';// 如果键名是一个字符串，则假定为 “字段名”=> “查询值” 这样的名值对
	...
	public function qualifyTable($sTableName,$sSchema=null,$sAlias=null){
		...
		if(strtoupper($sSchema)=='PUBLIC'){$sSchema='';}// public 是默认的schema
		$sI=$sSchema !=''?"\"{$sSchema}\".\"{$sTableName}\"":"\"{$sTableName}\"";
		return empty($sAlias)?$sI:$sI." \"{$alias}\"";
	}
	...
	public function parseSqlArray_($sTableName,array $arrValue,array $arrArgs){
		static $arrKeywords=array('('=>true,'AND'=>true,'OR'=>true,'NOT'=>true,'BETWEEN'=>true,'CASE'=>true,'&&'=>true,'||'=>true,'='=>true,'<=>'=>true,'>='=>true,'>'=>true,'<='=>true,'<'=>true,'<>'=>true,'!='=>true,'IS'=>true,'LIKE'=>true);
		$arrParts=array();
		$sNextOp='';
		$nArgsCount=0;
		$arrUsedTables=array();
	...
	public function fakeBind($sSql,$arrInput){
		if($arrInput===null) $arrInput=array();// 分析‘?’ 占位符
