...
class Db{
	...
	public function __construct($Dsn=null){
		self::$_arrDsn=$this->parseConfig($Dsn);
		...
		self::$_oFactory=new $sFactoryName();// 创建工厂&开始连接
	...
	static public function createDbInstance($Dsn=null,$sId=null ,$bDefaultIns=true,$bConnect=true){
		if($bDefaultIns and self::$_oDefaultDbIns){// 如果默认数据库对象存在，又选用默认的，则直接返回
			return self::$_oDefaultDbIns;
		}
		$oDb=new self($Dsn);// 创建一个数据库Db对象
		if($bConnect){// 连接
	...
	protected function parseConfig($Config=''){
		$arrDsn=array();
		if(is_array($Config) && !E::oneImensionArray($Config)){
			$arrDsn=$Config;
		}
		elseif(!empty($Config['db_dsn'])){// 如果DSN字符串则进行解析
			$arrDsn[]=$this->parseDsn($Config['db_dsn']);
		}
		elseif(is_array($Config) && E::oneImensionArray($Config)){
			$arrDsn[]=$Config;
		}
		...
		if(!$GLOBALS['_commonConfig_']['DB_DISTRIBUTED']){
			...
			foreach($arrDsn as $nKey=>&$arrValue){// 最后再次对数据进行默认填充判断
	...
	protected function fillFull($arrConfig=array()){
		return array_merge($arrConfig,array('db_type'=>$GLOBALS['_commonConfig_']['DB_TYPE'],'db_schema'=>$GLOBALS['_commonConfig_']['DB_SCHEMA'],'db_user'=>$GLOBALS['_commonConfig_']['DB_USER'],'db_password'=>$GLOBALS['_commonConfig_']['DB_PASSWORD'],'db_host'=>$GLOBALS['_commonConfig_']['DB_HOST'],'db_port'=>$GLOBALS['_commonConfig_']['DB_PORT'],'db_name'=>$GLOBALS['_commonConfig_']['DB_NAME'],'db_prefix'=>$GLOBALS['_commonConfig_']['DB_PREFIX'],'db_dsn'=>$GLOBALS['_commonConfig_']['DB_DSN'],'db_params'=>$GLOBALS['_commonConfig_']['DB_PARAMS']));
	}

	protected function parseDsn($sDsn){
		if(empty($sDsn)){// dsn为空，直接返回
			return false;
		}
		$arrInfo=parse_url($sDsn);// 分析dsn参数
		if($arrInfo['scheme']){// 解析数据
			$arrDsn=array(
	...
	public function update($sTableName,$Row,array $Where=null,$Limit='',$Order='',array $RstrictedFields=null){
		...
		if(!is_array($Row) && !($Row instanceof DbExpression)){
			G::E(G::L('$arrRow的格式只能是数组和DbException的实例。','DbDyhb'));
		}
		if(!is_array($Row)){
