...
class Image{
	...
	static public function thumb($sImage,$sThumbName,$sType='',$nMaxWidth=200,$nMaxHeight=50,$bInterlace=true){
		$arrInfo=self::getImageInfo($sImage);// 获取原图信息
		if($arrInfo!==false){
			...
			if($sType!='gif' && function_exists('imagecreatetruecolor')){$oThumbImg=imagecreatetruecolor($nWidth,$nHeight);}// 创建缩略图
			else $oThumbImg=imagecreate($nWidth,$nHeight);
			if(function_exists("ImageCopyResampled")){imagecopyresampled($oThumbImg,$oSrcImg,0,0,0,0,$nWidth,$nHeight,$nSrcWidth,$nSrcHeight);}// 复制图片
			else imagecopyresized($oThumbImg,$oSrcImg,0,0,0,0,$nWidth,$nHeight,$nSrcWidth,$nSrcHeight);
			...
			if('jpg'==$sType || 'jpeg'==$sType){imageinterlace($oThumbImg,$bInterlace);}// 对jpeg图形设置隔行扫描
	...
	static public function imageWaterMark($sBackgroundPath,$nWaterPos=0,$arrWaterArgs,$bDeleteBackgroupPath=true){
		...
		if(!empty($sBackgroundPath) && file_exists($sBackgroundPath)){// 读取背景图片
			...
			switch($arrBackgroundInfo[2]){// 取得背景图片的格式
				...
				default:
					return G::L("错误的图像格式！",'NetDyhb');
			}
		}
		else{
			return G::L("图像%d为空或者不存在！",'NetDyhb',null,$sBackgroundPath);
		}
		...
		if(!empty($sBackgroundPath) && file_exists($sBackgroundPath)){
			...
			elseif($arrWaterArgs['type']==='text' && $arrWaterArgs['content'] !=''){
				...
				if(isset($arrWaterArgs['textPath']) && strtolower($arrWaterArgs['textPath'])=='framework-font'){// 使用框架内部提供的字体
					$arrWaterArgs['textPath']=DYHB_PATH.'/Resource/Seccode/Fonts';
					$bFrameworkFont=true;
				}
				...
				if(!is_file($sFontfile)){
					return G::L("字体文件%s无法找到！",'NetDyhb',null,$sFontfile);
				}
				...
				return G::L("水印参数type不为img 和 text！",'NetDyhb');
			}
		}
		else{
			return G::L("水印参数不为数组！",'NetDyhb');
		}
	...
	static public function resizeImage($sImageFile,$sTargetFile='',$nWidth=80,$nHeight=80,$bCut=true,$sCorner='',$nCornerRadius=6,$nAngle=0){
		...
		$oIm=null;// 初始化图象
		if($sType=="jpg") $oIm=imagecreatefromjpeg($sImageFile);
		if($sType=="gif") $oIm=imagecreatefromgif($sImageFile);
		if($sType=="png") $oIm=imagecreatefrompng($sImageFile);
		if($oIm===null) return G::L("错误的图像格式！",'NetDyhb');
		if($sTargetFile===''){
			$nFullLength=strlen($sImageFile);// 图象目标地址
		...
		if($sTargetFile==$sImageFile) return G::L("裁剪的图像和原图像地址一样！",'NetDyhb');
		if(is_file($sTargetFile)) return G::L("保存的图像已经存在！",'NetDyhb');
		$nSourceWidth=imagesx($oIm);// 取得图像地址
		$nSourceHeight=imagesy($oIm);
		$nResizeRatio=($nWidth)/($nHeight);// 改变后的图象的比例
		if($sCorner=='') $sCorner=DYHB_PATH.'/Resource/Images/rounded_corner.png';// 初始化圆角信息
