...
class Seccode{
	...
	public function __construct($arrOption=null){
		$this->_arrDefaultOption['seccode_fontpath']=DYHB_PATH.'/Resource/Seccode/Fonts';
		$this->_arrDefaultOption['seccode_datapath']=DYHB_PATH.'/Resource/Seccode';
		if(!empty($arrOption)){
		...
		return $this->setWidth($arrDefaultOption['seccode_image_width_size'])// 验证码宽度
			...
			->setType($arrDefaultOption['seccode_tupe'])// 设置创建验证码文字的方式 1：基于字体来创建 2：基于图片来创建文字
			...
			->setFontSize($arrDefaultOption['seccode_fontsize'])// 验证码字体大小(px)
	...
	public function setCode($nCode){
		if(empty($nCode) or $nCode==0){return $this;}
		$this->_nCode=$nCode;
		return $this;
	}

	public function setWidth($nWidth){
		if(empty($nWidth) or $nWidth==0){return $this;}
		$this->_nWidth=$nWidth;
		return $this;
	}

	public function setHeight($nHeight){
		if(empty($nHeight) or $nHeight==0){return $this;}
	...
	public function setDataPath($sDataPath){
		if(empty($sDataPath)){return $this;}
	...
	public function setType($nType){
		if(!in_array($nType,array(1,2)))return $this;
		$this->_nType=$nType;
		return $this;
	}
	...
	public function setFontSize($nFontSize){
		if(empty($nFontSize)){return $this;}
	...
	public function display(){
		$nMaxWidthSize=self::SECCODE_IMAGE_WIDTH_MAX_SIZE;
		$nMaxHeightSize=self::SECCODE_IMAGE_HEIGHT_MAX_SIZE;
		$this->_nWidth=$this->_nWidth >=0 && $this->_nWidth<=$nMaxWidthSize?$this->_nWidth:$nMaxWidthSize;// 初始化高和宽度
		$this->_nHeight=$this->_nHeight>=0 && $this->_nHeight<=$nMaxHeightSize?$this->_nHeight:$nMaxHeightSize;
		E::seccodeConvert($this->_nCode,$this->_bChineseCode,$this->_nType);// 转码
		if( function_exists('imagecreate') and function_exists('imagecolorset') and function_exists('imagecopyresized') and function_exists('imagecolorallocate') and function_exists('imagesetpixel') and function_exists('imagechar') and function_exists('imageline') and function_exists('imagecreatefromstring') and (function_exists('imagegif') or function_exists('imagepng') or function_exists('imagejpeg'))){
			$this->image();
		}
		else{
			$this->bitMap();
	...
	protected function image(){
		...
		if($this->_bAnimator===TRUE && function_exists('imagegif')){// 是否开启动画验证码
			...
			for($nI=0;$nI<=9;$nI++){
				...
				if($nI==$nTrueFrame){
					$this->_bTtf && function_exists('imagettftext') || $this->_nType== 1?$this->ttfFont():$this->gifFont();
					$this->gifFont();
	...
	protected function writeNoise_(){
		for($nI=0;$nI<10;$nI++){
			...
			for($nJ=0;$nJ<5;$nJ++){
				imagestring($this->_oIm,5,mt_rand(-10,$this->_nWidth),mt_rand(-10,$this->_nHeight),$this->_arrCodeSet[mt_rand(0, 27)], /* 杂点文本为随机的字母或数字 */$noiseColor);// 绘杂点
			}
		}
	}
	...
	protected function background(){
		...
		if($this->_bBackground && $this->_bImageBackground && function_exists('imagecreatefromjpeg') && function_exists('imagecolorat') && function_exists('imagecopymerge') && function_exists('imagesetpixel') && function_exists('imageSX') && function_exists('imageSY')){
			if($hHandle=@opendir($this->_sDataPath.'/background')){
				while($sBgFile=@readdir($hHandle)){
			...
			if($arrBackgrounds){
				...
				$arrC[1]=$arrC['green'];
				$arrC[2]=$arrC['blue'];
				imagecopymerge($this->_oIm,$oImwm,0,0,mt_rand(0,200-$this->_nWidth),mt_rand(0,80-$this->_nHeight),imageSX($oImwm),imageSY($oImwm),100);// imagecopymerge函数支持png透明拷贝
				imagedestroy( $oImwm);
			}
		}
		if(!$this->_bBackground || !$arrBackgrounds){
			...
			for($nI=0;$nI<$this->_nWidth;$nI++){
				...
				imageline($this->_oIm,$nI,0,$nI-(isset($bTilt)?$bTilt:0),$this->_nHeight,$oColor);
				$arrC[0]+=$arrStep[0];
				$arrC[1]+=$arrStep[1];
				$arrC[2]+=$arrStep[2];
			}
	...
	protected function adulterate(){
		...
		for($nI=0;$nI<=$nLineNums;$nI++){
			$oColor=$this->_bColor?imagecolorallocate($this->_oIm,mt_rand(0,255),mt_rand(0,255),mt_rand(0,255)):imagecolorallocate($this->_oIm,$this->_arrFontColor[0],$this->__arrFontColor[1], $this->_arrFontColor[2]);
	...
	protected function ttfFont(){
		...
		$sSeccodeRoot=$this->_sFontPath.($this->_bChineseCode===true?'/Zh-cn':'/En' ); // 字体路径
		$hDirs=opendir($sSeccodeRoot);// 打开验证码句柄
		$arrSeccodeTtf=array();
		while($entry=readdir( $hDirs)){
			if($entry!='.' and $entry!='..' and in_array(strtolower($this->fileExt($entry)),array('ttf','ttc'))){
				$arrSeccodeTtf[]=$entry;
			}
		}
		...
		for($nI=0;$nI<$nSeccodeLength;$nI++){
			...
			$arrFont[$nI]['size']=$this->_nWidth/6;// 大小 1/6图像大小( $this->_nFontSize一边凉快去？)
		...
		$nX=mt_rand($arrFont[0]['tilt']>0?cos(deg2rad(90-$arrFont[0]['tilt']))*$arrFont[0]['zheight']:1,$this->_nWidth-$nWidthTotal);// deg2rad()函数将角度转换为弧度&cos是cosine的简写.表示余弦函数
		!$this->_bColor and $oTextColor=imagecolorallocate($this->_oIm,$this->_arrFontColor[0],$this->_arrFontColor[1],$this->_arrFontColor[2]); // 是否启用随机颜色
		for($nI=0;$nI<$nSeccodeLength;$nI++){
			if($this->_bColor){
	...
	protected function bitmap(){
		...
		for($nI=0;$nI<20;$nI++){
			for($nJ=0;$nJ<=3;$nJ++){
				$nA=mt_rand(0,14);
				if(isset($arrNumbers[$this->_nCode[$nJ]])){
		...
		$sImage=pack('H*','424d9e000000000000003e000000280000002000000018000000010001000000'.'0000600000000000000000000000000000000000000000000000FFFFFF00'.implode('',$arrBitmap));
