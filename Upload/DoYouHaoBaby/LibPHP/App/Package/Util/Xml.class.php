...
class Xml{
	...
	static public function xmlSerialize(&$arrData,$bHtmlOn=false,$nLevel=0,$sPriorKey=NULL,$sCharset='UTF-8'){
		...
		while(list($sKey, $sValue)=each($arrData)){
			if(!strpos($sKey,' attr')){
				if(is_array($sValue)and array_key_exists(0,$sValue)){
					...
					if(array_key_exists("$sKey attr",$arrData)){
						while(list($sAttrName,$sAttrValue)=each($arrData["$sKey attr"])){echo ' ',$sAttrName,'="',($bHtmlOn?'<![CDATA[':'').$sAttrValue.($bHtmlOn?']]>':''),'"';}
						reset($arrData["$sKey attr"]);
					}

				if(is_null($sValue)){echo " />\n";}
				elseif(!is_array($sValue)){echo '>',($bHtmlOn?'<![CDATA[':'').$sValue.($bHtmlOn?']]>':''),"</$sTag>\n";}
	...
	public function open(&$oParser,$sTag,$arrAttributes){
		...
		if($arrAttributes)$this->_arrParent["$nKey attr"]=$arrAttributes;
		$this->_arrParent =&$this->_arrParent[$nKey];
		$this->_arrStack[]=&$this->_arrParent;
	}

	public function data(&$oParser,$sData){
		if($this->_sLastOpenedTag!=NULL){$this->_sData.=$sData;}
	}
	public function close(&$oParser,$sTag){
		...
		if($this->_arrStack)$this->_arrParent=&$this->_arrStack[count($this->_arrStack)-1];
	}
}
