...
class GifMerge{
	...
	protected function startGifmergeProcess($sFp){
		if($this->_sMod==self::C_FILE){// 使用文件模式储存临时数据
		...
		$this->getBytes(6);// 取得字节为6的地方的数据，用于判断是否为gif数据
		if(!$this->arrCmp($this->_arrBuffer,$this->_arrGif,3)){
			return;
		}
		$this->getBytes(7);// 取得长度为7的数据
		if($this->_bFirst){$this->_arrLogicalScreenDescriptor=$this->_arrBuffer;}// 如果为开始的地方
	...
	protected function readImageDescriptor(){
		$this->getBytes(9);// 取得9的长度字节内容
		$arrHead=$this->_arrBuffer;
		$this->_nLocalColorTableFlag=($this->_arrBuffer[8]&0x80)?true:false;// 本地色彩类型
