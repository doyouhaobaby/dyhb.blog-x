...
class Mail{
	...
	public function send(){
		...
		if($this->_bIsHtml===TRUE) $this->_sContentType='text/html';// 是否允许HTML 代码
		$sHeaders="From:{$sEmailFrom}{$sEmailLimiter}X-Priority: 3{$sEmailLimiter}X-Mailer: DoYouHaoBaby!{$sEmailLimiter}MIME-Version: 1.0{$sEmailLimiter}Content-type: $this->_sContentType; charset={$this->_sCharset}{$sEmailLimiter}Content-Transfer-Encoding: base64{$sEmailLimiter}";// 邮件头部
		...
		if(strtolower($this->_sEmailSendType)==self::PHP_MAIL && function_exists('mail')){
			...
			fputs($hFp,($this->_bAuth?'EHLO':'HELO')." DoYouHaoBaby\r\n");
			$sLastMessage=fgets($hFp,512);
			...
			fputs($hFp,"MAIL FROM: <".preg_replace("/.*\<(.+?)\>.*/","\\1",$this->_sEmailFrom).">\r\n");
			$sLastMessage=fgets($hFp,512);
			...
			foreach($arrEmailTo as $sToUser){
				...
				if($sToUser){
					fputs($hFp,"RCPT TO: <".preg_replace("/.*\<(.+?)\>.*/","\\1",$sToUser).">\r\n");
					$sLastMessage=fgets($hFp,512);
					if(substr($sLastMessage,0,3)!=250){
						fputs($hFp,"RCPT TO: <".preg_replace("/.*\<(.+?)\>.*/","\\1",$sToUser).">\r\n");
						$sLastMessage=fgets($hFp,512);
	...
	public function setEmailLimiter($nEmailLimiter=1){
		if(!in_array($nEmailLimiter,array(1,2,3))) return;
