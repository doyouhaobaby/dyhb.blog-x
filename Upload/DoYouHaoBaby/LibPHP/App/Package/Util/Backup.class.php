...
class Backup{
	...
	static public function getHead($sPath){
		$sSqlInfo=array('os'=>'','date'=>'','ver'=> '','php_ver'=>0,'vol'=>0,'database_ver'=>0);// 获取Sql文件头部信息
		$hFp=fopen($sPath,'rb');
		$sStr=fread($hFp,250);
		fclose($hFp);
		$arrInfo=explode("\n",$sStr);
	...
	protected function qualifyStr($sValue){
		$oDbQualifyStr=DbConnect::getFactory()->createDbQualifyStr($this->_oDbConnect,$sValue);
