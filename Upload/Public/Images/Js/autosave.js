...
var AutoHideMsg		= 55;
var autosave_time		= D.L('秒后自动保存');
var autosave_wait		= D.L('正在保存...');
var autosave_ok			= D.L('已自动保存');
var autosave_error		= D.L('自动保存错误');
var autosave_disable	= D.L('禁止自动保存');
...
function saveDraft(){
	var blog_id=document.getElementById('id').value;
	...
	var gourl=D.U('autosave');

	if(blog_title==''){
		blog_title=D.L('未命名的标题');
	}
	if(blog_content==''){
		blog_content='&nbsp;';
	}

	var postData='blog_id='+blog_id+'&blog_title='+encodeURIComponent(blog_title)+'&blog_content='+encodeURIComponent(blog_content);
...
function saveMydraft(){
	...
	if(http_request.readyState == 4){
		if(http_request.status == 200){
			$('#id').val(http_request.responseText);
			var oDigital=new Date();
			var hours=oDigital.getHours();
			var mins=oDigital.getMinutes();
			var secs=oDigital.getSeconds();
			timemsg2.innerHTML=autosave_ok+hours+":"+mins+":"+secs;
			savetime=-1000;
		}	else {
